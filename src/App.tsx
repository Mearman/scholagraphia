...
import { getEntityDetails, getRelatedEntities, searchEntities } from './api/openAlex';
...
const AppContent: React.FC = () => {
  ...
  const performSearch = useCallback(async (query: string, entityType: string) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  const handleNewSearch = (results: SearchResult[], query: string, entityType: string) => {
    setSearchResults(results);
    setIsLoading(false);
    setCurrentQuery(query);
    setCurrentEntityType(entityType);
  ...
  const updatePageTitle = (query: string) => {
    document.title = query ? `${query} - OpenAlex Explorer` : 'OpenAlex Explorer';
  };
  ...
  const handleShowRelated = useCallback(async (entityId: string) => {
    ...
    try {
      ...
      setError('An error occurred while fetching related entities. Please try again.');
    } finally {
      setIsLoading(false);
    }
  }, []);
