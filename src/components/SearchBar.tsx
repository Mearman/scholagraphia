...
import { Search, ToggleLeft, ToggleRight, ChevronDown, X } from 'lucide-react';
import { searchEntities } from '../api/openAlex';
import { useAppContext } from '../context/AppContext';
import { SearchResult } from '../types';

interface SearchBarProps {
  onNewSearch: (results: SearchResult[], query: string, entityType: string) => void;
  setIsLoading: (isLoading: boolean) => void;
  initialQuery: string;
  initialEntityType: string;
}

const SearchBar: React.FC<SearchBarProps> = ({ onNewSearch, setIsLoading, initialQuery, initialEntityType }) => {
  const [query, setQuery] = useState(initialQuery);
  const [entityType, setEntityType] = useState(initialEntityType);
  const { setSearchResults, searchWhileTyping, toggleSearchWhileTyping } = useAppContext();
  ...
  const performSearch = useCallback(async (searchQuery: string, type: string) => {
    ...
    if (lastSearchRef.current?.query === searchQuery && lastSearchRef.current?.type === type) {
  ...
  const debouncedSearch = useCallback((searchQuery: string, type: string) => {
    ...
    <div className="w-full max-w-3xl">
      ...
      <div className="flex items-center justify-end text-sm text-gray-600 dark:text-gray-400">
        ...
        <button
          ...
          aria-label={searchWhileTyping ? "Disable search while typing" : "Enable search while typing"}
        >
          {searchWhileTyping ? (
