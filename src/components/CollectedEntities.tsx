...
import { Upload, Download, Trash2, MoreVertical, FileUp, FileDown } from 'lucide-react';
...
const CollectedEntities: React.FC<CollectedEntitiesProps> = ({ onShowRelated }) => {
  const { 
    collections,
    setCollections,
    activeCollectionId,
  ...
  const handleImport = (event: React.ChangeEvent<HTMLInputElement>, replace: boolean) => {
  ...
  return (
    <div className="mt-8">
      ...
      {activeCollection && activeCollection.entities.map(entity => (
        <EntityCard
          key={entity.id}
          entity={entity}
      ...
      {(!activeCollection || activeCollection.entities.length === 0) && (
        <p className="text-gray-600 dark:text-gray-400">No entities collected in this collection yet.</p>
      )}
    </div>
  );
};
