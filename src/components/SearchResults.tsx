...
const SearchResults: React.FC<SearchResultsProps> = ({ searchResults = [], onShowRelated, isLoading }) => {
  ...
  const handleCollect = async (result: SearchResult) => {
    try {
      ...
      const normalizedType = result.entity_type.endsWith('s') ? result.entity_type.slice(0, -1) : result.entity_type;
      ...
      const newEntity: CollectedEntity = { ...entityDetails, related_nodes: relatedNodes };
      ...
      let errorMessage = 'An unknown error occurred while collecting the entity.';
  ...
  const handleRemove = (id: string) => {
    setCollections(prevCollections => 
      prevCollections.map(collection => 
        collection.id === activeCollectionId
          ? { ...collection, entities: collection.entities.filter(entity => entity.id !== id) }
          : collection
      )
    );
  ...
  const isCollected = (id: string) => {
    ...
    return activeCollection ? activeCollection.entities.some(entity => entity.id === id) : false;
  };

  return (
    <div className="mt-4">
      <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">Search Results</h2>
