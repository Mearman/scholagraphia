...
export const decodeAndDecompressIds = (encodedIds: string): string[] => {
  const compressedData = atob(encodedIds).split('').map(char => char.charCodeAt(0));
  const decompressed = pako.inflate(new Uint8Array(compressedData), { to: 'string' });
  return decompressed.split(',');
};

export const generateShareableLink = (ids: string[], collectionTitle: string): string => {
...
export const generateShareableMultiCollectionLink = (collections: { name: string; ids: string[] }[]): string => {
  const encodedCollections = btoa(JSON.stringify(collections));
  const currentUrl = window.location.href.split('?')[0];
  return `${currentUrl}?sharedCollections=${encodedCollections}`;
};

export const parseSharedIds = (url: string): { ids: string[] | null, title: string | null, collections: { name: string; ids: string[] }[] | null } => {
