...
import { AppContextType, AppProviderProps, SearchResult, CollectedEntity, Entity, ThemeMode, Collection } from '../types';
import { parseSharedIds, mergeSharedIds, generateShareableMultiCollectionLink } from '../utils/idSharing';
...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
  ...
  const [collections, setCollections] = useState<Collection[]>(() => {
    ...
    return savedCollections ? JSON.parse(savedCollections) : [{ id: '1', name: 'Default Collection', entities: [] }];
  });
  ...
  useEffect(() => {
    localStorage.setItem('searchWhileTyping', JSON.stringify(searchWhileTyping));
  }, [searchWhileTyping]);
  ...
  useEffect(() => {
    const { ids: sharedIds, title: sharedTitle, collections: sharedCollections } = parseSharedIds(window.location.href);
    
    if (sharedCollections) {
      const newCollections = sharedCollections.map((sharedCollection, index) => ({
        ...
        collection.entities.forEach(async (entity) => {
          try {
            ...
            switch (entityType.toLowerCase()) {
              case 'w': type = 'work'; break;
              case 'a': type = 'author'; break;
              case 'v': type = 'venue'; break;
              case 'i': type = 'institution'; break;
              case 'c': type = 'concept'; break;
              case 'p': type = 'publisher'; break;
              case 's': type = 'source'; break;
              case 'f': type = 'funder'; break;
              case 'g': type = 'country'; break;
                ...
                c.id === collection.id 
                  ? { 
                      ...
                      entities: c.entities.map(e => 
                        e.id === entity.id ? { ...entityDetails, related_nodes: relatedNodes } : e
                      ) 
                    } 
                  : c
              )
      ...
      if (sharedTitle) {
        targetCollection = collections.find(c => c.name === sharedTitle) || 
          { id: Date.now().toString(), name: sharedTitle, entities: [] };
        if (!collections.some(c => c.id === targetCollection.id)) {
          setCollections(prevCollections => [...prevCollections, targetCollection]);
        }
      } else {
        targetCollection = collections.find(c => c.id === activeCollectionId) || collections[0];
      }
      
      const mergedEntities = mergeSharedIds(targetCollection.entities, sharedIds);
      setCollections(prevCollections => 
        prevCollections.map(c => 
  ...
  const importAndMergeCollections = (importedCollections: Collection[]) => {
    setCollections(prevCollections => {
      ...
      importedCollections.forEach(importedCollection => {
        const existingCollectionIndex = mergedCollections.findIndex(c => c.name === importedCollection.name);
        if (existingCollectionIndex !== -1) {
          mergedCollections[existingCollectionIndex].entities = [
            ...mergedCollections[existingCollectionIndex].entities,
            ...importedCollection.entities.filter(entity => 
              !mergedCollections[existingCollectionIndex].entities.some(e => e.id === entity.id)
            )
          ];
        } else {
  ...
  const importAndReplaceCollections = (importedCollections: Collection[]) => {
    setCollections(importedCollections.map(collection => ({
      ...collection,
      id: Date.now().toString() + Math.random().toString(36).substr(2, 9)
    })));
  ...
  const mergeCollections = (collectionIds: string[]) => {
    const collectionsToMerge = collections.filter(c => collectionIds.includes(c.id));
    const mergedEntities = collectionsToMerge.flatMap(c => c.entities);
    const uniqueEntities = Array.from(new Set(mergedEntities.map(e => e.id)))
  ...
  const splitCollection = (collectionId: string, entityIds: string[]) => {
    ...
    if (collectionToSplit) {
      const remainingEntities = collectionToSplit.entities.filter(e => !entityIds.includes(e.id));
      const splitEntities = collectionToSplit.entities.filter(e => entityIds.includes(e.id));
